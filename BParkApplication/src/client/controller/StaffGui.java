...
public class StaffGui implements ChatIF {
	...
	/**
	 * Handles the action for viewing the parking database. Sends a request to the
	 * server and appends status to the UI.
	 */
	@FXML
	void ViewDB() {
		String command = "VIEW_DATABASE";
		outputDisplay.setText("Fetching data from DataBase...\n");
		try {
	...
	void addNewUser() {
		...
		if (name == null || name.isEmpty() || id == null || id.isEmpty() || email == null || email.isEmpty()
	...
	public StackPane buildRoot() {
		...
		HBox nameIdBox = new HBox(10, nameBox, idBox, spacer, logOutBtn);
		...
		VBox Interior = new VBox(5, nameIdBox, phoneEmailAddBox, viewlogBox, outputDisplay, table);
		Interior.setMargin(viewlogBox, new Insets(10, 0, 0, 0));
